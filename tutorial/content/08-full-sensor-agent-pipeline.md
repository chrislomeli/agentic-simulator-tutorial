...
```python
"""
Tools available to the classify LLM during the ReAct loop.

Each tool receives sensor event data injected at call time via closure.
The LLM calls these to structure raw data before reasoning.
"""
...
def make_classify_tools(events: list):
    ...
    Called at the start of each classify node invocation so the tools
...
def make_classify_node(registry, tools: list):
    ...
    def classify(state: ClusterAgentState) -> dict:
        ...
        try:
            ...
            logger.warning("classify: failed to parse LLM response: %s", exc)
            return {
                "messages": [response],
                "anomalies": [],
                "status": StatusValue.PROCESSING,
...
```python
tools = make_classify_tools([])          # for ToolNode (schema only; classify rebinds)
