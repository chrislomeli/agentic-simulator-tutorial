...
def _make_state(**overrides) -> ClusterAgentState:
    """Build a minimal ClusterAgentState for testing."""
    defaults = {
    ...
    return ClusterAgentState(**defaults)
...
class TestReportFindings:
    def test_returns_empty_update(self):
        state = _make_state(anomalies=[AnomalyFinding(
            finding_id="f1",
            cluster_id="c1",
            anomaly_type="test",
            affected_sensors=[],
...
class TestRouteAfterClassify:
    ...
    def test_routes_to_end_on_error(self):
        state = _make_state(status=StatusValue.ERROR, error_message="boom")
