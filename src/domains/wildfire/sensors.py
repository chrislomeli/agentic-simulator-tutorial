"""
world-simiulator.domains.wildfire.sensors
...
class WindSensor(SensorBase):
    ...
    def read(self, local_conditions: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        direction = (
            lc.get("wind_direction_deg", 0.0) + random.gauss(0, self._direction_noise_std)
        ) % 360.0
        return {
...
class SmokeSensor(SensorBase):
    ...
    def read(self, local_conditions: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        for fire in nearby_fires:
            ...
            contribution = fire["intensity"] * distance_factor * wind_factor * speed_factor * 80.0
...
class ThermalCameraSensor(SensorBase):
    ...
    def read(self, local_conditions: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not heat_grid:
            heat_grid = [
                [
                    round(ambient + random.gauss(0, self._noise_std), 1)
                    for _ in range(self._view_cols)
                ]
