...
All instance raw lives in raw/regions/*.json. Adding a new region:
  1. Create raw/regions/{name}.json (bbox, grid, clusters, stations,
     calibration reference).
  2. Ensure the referenced calibration exists in raw/calibrations/.
...
_REGION_DATA_DIR = Path(__file__).parent / "raw" / "regions"
...
class RegionProfile:
    ...
    Loaded from raw/regions/{name}.json — use get_region() to retrieve.
    """
