...
def _trace_sector(
    ...
    """
    from domains.wildfire.cell_state import TerrainType, FireState
...
def make_logistics_agent_node(tools: list, prompt_registry: PromptRegistry, llm_registry: LLMRegistry | None):
    ...
    def logistics_agent(state: LogisticsAgentState) -> dict:
        ...
        print(f"""\n{blue}● {json.dumps(state.sector_analysis)} {reset}""")

        print(f"""\n{green}● {json.dumps(state.situation_summary)} {reset}""")

        if STUB_LOGISTICS or llm_with_tools is None:
            stub = AIMessage(content="[STUB] Logistics plan — LLM not active in this milestone.")
            return {"messages": [stub], "status": StatusValue.PROCESSING}

        # this is not a stub - we are calling the logistics agent
        ...
        if not messages:
            content = (
                ...
                "hotspot and produce a deployment plan."
            )
...
def make_extract_plan_node(llm_registry: LLMRegistry | None):
    ...
    if llm_registry is not None:
        try:
            ...
            extraction_llm = llm_registry.get("logistics").with_structured_output(LogisticsAssessment)
        except KeyError:
            logger.warning("No 'logistics' LLM registered — assessment extraction skipped.")
    ...
    def extract_logistics_plan(state: LogisticsAgentState) -> dict:
        ...
        assessment = None
        if extraction_llm is not None and state.messages:
            from langchain_core.messages import HumanMessage as _HumanMessage
            extraction_messages = list(state.messages) + [
                _HumanMessage(
                    content=(
                        "Based on the conversation above, extract a structured assessment. "
                        "In observations, list only what the data directly showed. "
                        "In data_gaps, name each specific input that was missing or "
                        "unavailable — be concrete, not general."
                    )
                )
            ]
            assessment = extraction_llm.invoke(extraction_messages)
            logger.info(
                "Assessment extracted — gaps=%d  advisory_sent=%s",
                len(assessment.data_gaps),
                assessment.advisory_sent,
            )
