"""HeatMapLayer — grid-aligned risk state for supervisor/resource planning.
...
if TYPE_CHECKING:
    from agents.cluster.schemas import CollatedRecordRisk
...
class CellRiskState:
    ...
    def is_hotspot(self, threshold: int = 7) -> bool:
        """Check if this cell exceeds risk threshold."""
        return self.risk_score >= threshold and self.confidence >= 2
...
class HeatMapLayer:
    ...
    def __init__(self, rows: int, cols: int, layers: int = 1) -> None:
        ...
        logger.info("Initialized HeatMapLayer: %dx%dx%d cells (all baseline risk=0)", 
                   rows, cols, layers)
        ...
        for layer in self._grid:
            for row in layer:
                for cell in row:
                    ...
                    cell.mark_clean()
        logger.info("HeatMapLayer reset to baseline")


def create_heat_map_from_terrain(rows: int, cols: int, layers: int = 1) -> HeatMapLayer:
    """Factory: Create heat map matching terrain grid dimensions.
    
    Called by scenario_loader to ensure 1:1 mapping with terrain.
    """
    return HeatMapLayer(rows=rows, cols=cols, layers=layers)
