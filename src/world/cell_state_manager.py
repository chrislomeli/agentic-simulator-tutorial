...
class EvaluationThresholds:
    ...
    # Time-based — max seconds between evals for cells that have raw
...
class _CellSnapshot:
    ...
    # that contributed raw. TRADE-OFF: a cell can be touched by sensors from
    ...
    def should_evaluate(self, thresholds: EvaluationThresholds) -> bool:
        ...
        if thresholds.max_eval_interval_sec is not None:
            if self.last_evaluated_at is None:
                # Never evaluated but has raw → trigger
                return True
...
class CellStateManager:
    ...
    def snapshot_cluster(
        ...
        Returns a CollatedRecord per cell that has raw and belongs to
    ...
    def snapshot_halo(
        ...
        raw, grouped by their cluster_id. This provides spatial context
